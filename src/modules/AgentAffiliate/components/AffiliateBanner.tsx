...
export const AffiliateBanner: React.FC<AffiliateBannerProps> = ({ bannerType, className, source }) => {
    ...
    const bannerRef = useVisibilityTracker<HTMLDivElement>({
        onVisibleThreshold: () => {
            if (banner?.id) {
                incrementAffiliateView(banner.id);
            }
        },
        thresholdMs: 10000,
        enabled: !!banner?.id && !banner.id.startsWith('fallback-')
    });

    // Merge refs
    const setRefs = (el: HTMLDivElement | null) => {
        (containerRef as any).current = el;
        (bannerRef as any).current = el;
    };
    ...
    useEffect(() => {
        const loadBanner = async () => {
            ...
            if (banners.length > 0) {
                ...
                // DEFERRED: Tracking moved to useVisibilityTracker (10s threshold)
            } else {
    ...
    return (
        ...
        >
            <div ref={setRefs} className="w-full flex justify-center" />
        </div>
    );
};
